...
test('theme toggle switches and persists the dashboard theme', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    window.localStorage.setItem('flowboard_theme', 'light');
  });
  await page.reload();
  ...
  await expect(page.getByText('$148,450')).toBeVisible();
  await expect(page.getByText('3,420')).toBeVisible();
  await expect(page.getByText('Meridian Analytics')).toBeVisible();
